## This is a modified version of psoptuim function forom PSO package: it uses the objective function as if it is vectorized, without need for any apply(...,fn1,...) call.
...
psoptim <- function (par, fn, gr = NULL, ..., lower=-1, upper=1,
  ...
  #f.x <- apply(X,2,fn1,mode="vectorized") # first evaluations
  f.x <- fn1(t(X))
  stats.feval <- p.s
  P <- X
  f.p <- f.x
